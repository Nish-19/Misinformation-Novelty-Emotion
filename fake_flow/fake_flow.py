...
class fake_flow:
    ...
    def __init__(self, model_name, SHUFFLE=False):
        ...
        self.parameters = {'rnn_size': 8,
                           ...
                           'vocab': 1000000,
                           'embedding_path': '../resources/GoogleNews-vectors-negative300.bin',
                           'embeddingSize': 300,
                           'max_epoch': 50,
                           'batch_size': 8,
                           }
    ...
    def prep_embed(self):
        path = 'fake_flow'
        filename = '{}2.npy'.format(self.model_name.split('_')[0])
    ...
    def run_model(self, type_='train'):
        file = 'fake_flow/{}_{}2.check'.format(self.__class__.__name__, self.model_name)
        # 1, Compile the model
        ...
        # 2, Prep
        callback = [EarlyStopping(min_delta=0.0001, patience=10, verbose=2, restore_best_weights=True),
                    ReduceLROnPlateau(factor=0.03, patience=10, verbose=2, min_lr=0.00001)]
        if not self.search:
    ...
    def objective_function(self, params):
        ...
        df_summary_table.to_csv('/fake_flow/results_{}_{}2.csv'.format(self.__class__.__name__, self.model_name), header=True, index=False)
