...
class emotional_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = 'fake_flow/features/emotional_features2_{}_{}.npy'.format(self.model_name, self.representation)
        if exists(file_name):
...
class sentiment_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = 'fake_flow/features/sentiment_features2_{}.npy'.format(self.model_name)
        if exists(file_name):
            features = np.load(file_name).tolist()
        else:
            data = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes") \
...
class morality_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = 'fake_flow/features/morality_features2_{}.npy'.format(self.model_name)
        if exists(file_name):
            features = np.load(file_name).tolist()
        else:
            data = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes") \
...
class imageability_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = 'fake_flow/features/imageability_features2_{}.npy'.format(self.model_name)
        if exists(file_name):
            features = np.load(file_name).tolist()
        else:
            data = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes") \
...
class hyperbolic_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = 'fake_flow/features/hyperbolic_features2_{}.npy'.format(self.model_name)
        if exists(file_name):
            features = np.load(file_name).tolist()
        else:
            data = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes") \
