...
class Preprocessor(object):
    ...
    def read_data(self, filepath):
        ...
        """
        obj_df = pd.read_csv(filepath)
        ids, premises, hypotheses, labels = [], [], [], []
        print('Here', file =deb_file)
        ...
        for index, row in obj_df.iterrows():
            # print('Reached here', file = deb_file)
            # line = line.strip().split(",")
            # print(line, file = deb_file)
            ...
            #     print(line, file = deb_file)
            pair_id = row['id']
            premise = row['title1_en']
            hypothesis = row['title2_en']
